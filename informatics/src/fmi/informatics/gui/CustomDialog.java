...
public class CustomDialog extends JDialog implements ActionListener, PropertyChangeListener {
	...
	private ButtonGroup sortOrderButtons = new ButtonGroup();
	...
	public CustomDialog(String text, PersonDataGUI parent) {
		...
		sortOrderPanel.setBorder(BorderFactory.createTitledBorder(BorderFactory.createEtchedBorder(), "Please choose a sort type."));

		// Създаване на масив с текст и с компоненти, които да се визуализират
		Object[] array = {text, sortOrderPanel, textField};
		...
		if (isVisible() && (evt.getSource() == optionPane)
			...
			if (value.equals(okLabel)) {
				try {
					...
					JOptionPane.showMessageDialog(CustomDialog.this, "Please type a Number!", "Error! Invalid input!", JOptionPane.ERROR_MESSAGE);

					textField.setText("");
					typedText = null;
					textField.requestFocusInWindow();
					//e.printStackTrace(); //Коментирам го с цел просто да не показва постоянно грешки в конзолата
				}
			} else {
				typedText = null;
				clearAndHide();
